...
WEAK_TOPIC_SCORE_THRESHOLD = 55  # avg score below this for a topic in the label window = weak
...
async def build_dataset() -> pd.DataFrame:
    ...
    for student in students:
        ...
        for (_, subject, topic) in student_topic_keys:
            ...
            feat_exams = [e for e in all_exams_for_topic if e.exam_date < feature_cutoff]
            label_exams = [e for e in all_exams_for_topic if e.exam_date >= feature_cutoff]
            ...
            other_topic_scores = [
                float(e.score) for k, exams in exams_by_key.items()
                if k[0] == sid and k != (sid, subject, topic)
                for e in exams if e.exam_date < feature_cutoff
            ]
            student_overall_avg = np.mean(other_topic_scores) if other_topic_scores else topic_avg_score
            relative_gap = topic_avg_score - student_overall_avg  # negative = weaker than own average
...
def train_model(df: pd.DataFrame):
    ...
    feature_cols = [
        "attendance_rate", "homework_submission_rate",
        "topic_avg_score", "student_overall_avg", "relative_gap",
    ...
    print(classification_report(y_test, y_pred, target_names=["not_weak", "weak_topic"]))
...
async def main():
    ...
    if len(df) < 100:
        print(f"WARNING: only {len(df)} (student, topic) rows — may be too small for a reliable model.")
        return
    train_model(df)
