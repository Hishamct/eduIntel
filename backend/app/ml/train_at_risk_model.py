...
async def build_dataset() -> pd.DataFrame:
    ...
    if not attendance_rows:
        raise RuntimeError(
            "No attendance data found — run the seed_ml_dataset script first."
        )
    ...
    for student in students:
        ...
        exams_feat = [
            e for e in exams_by_student.get(sid, []) if e.exam_date < feature_cutoff
        ]
        doubts = [
            d
            for d in doubts_by_student.get(sid, [])
            if d.created_at.date() < feature_cutoff
        ]
        ...
        avg_score_early = (
            np.mean(
                [
                    float(e.score)
                    for e in exam_scores_sorted[: len(exam_scores_sorted) // 2]
                ]
            )
            if len(exam_scores_sorted) >= 2
            else float(exam_scores_sorted[0].score)
        )
        avg_score_late = np.mean(
            [float(e.score) for e in exam_scores_sorted[len(exam_scores_sorted) // 2 :]]
        )
        score_trend = (
            avg_score_late - avg_score_early
...
def train_model(df: pd.DataFrame):
    ...
    print(
        classification_report(y_test, y_pred, target_names=["not_at_risk", "at_risk"])
    )
...
async def main():
    ...
    if len(df) < 30:
        print(
            f"WARNING: only {len(df)} students have valid data — this is too small for a reliable model."
        )
        return
    train_model(df)
