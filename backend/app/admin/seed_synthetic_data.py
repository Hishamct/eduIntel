...
from app.core.database import (
    AsyncSessionLocal,  # adjust import if your session factory is named differently
)
    ...
    async with AsyncSessionLocal() as db:
        ...
        for student in students:
            ...
            for m in range(MONTHS_BACK):
                due_date = (
                    today.replace(day=1) - datetime.timedelta(days=30 * m)
                ).replace(day=5)
                ...
                if roll < 0.75:
                    status = "paid"
                    paid_date = due_date + datetime.timedelta(
        ...
        print(
            f"Seeded {len(fee_records)} fee records and {len(attendance_records)} attendance records for {len(students)} students."
        )
